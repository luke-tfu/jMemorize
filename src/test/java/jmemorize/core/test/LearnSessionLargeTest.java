...
public class LearnSessionLargeTest extends TestCase {
    ...
    private void startSession() {
        m_session = new DefaultLearnSession(m_category, m_settings, new ArrayList<Card>(), true, true, m_provider);

        m_session.startLearning();
    }

    private void assertSummary(boolean isEnded, int passed, int failed, int skipped, int relearned) {
        String actual = createSummaryString(m_provider.isSessionEnded(), m_session.getPassedCards().size(),
                m_session.getFailedCards().size(), m_session.getSkippedCards().size(),
                m_session.getRelearnedCards().size());

        String expected = createSummaryString(isEnded, passed, failed, skipped, relearned);

        assertEquals(expected, actual);
    }

    private String createSummaryString(boolean isEnded, int passed, int failed, int skipped, int relearned) {
