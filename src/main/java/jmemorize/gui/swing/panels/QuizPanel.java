 ...
 * @author djemili
 */
public class QuizPanel extends JPanel implements Events, LearnCardObserver, CategoryObserver, LearnSessionObserver {
    private abstract class AbstractLearnAction extends AbstractAction2 {
        ...
        protected void setFocusedWindowShortcut(String action, char key) {
            getInputMap(JButton.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(key), action);
    ...
    private void updateCardSidePanels() {
        if (m_currentCard != null) {
            CardSide questionSide = !m_showFlipped ? m_currentCard.getFrontSide() : m_currentCard.getBackSide();

            CardSide answerSide = !m_showFlipped ? m_currentCard.getBackSide() : m_currentCard.getFrontSide();
    ...
    private void showQuestion() {
        if (m_isShowAnswer) {
            m_frontWasVisible = m_questionCardPanel.isCardSideVisible(0) && m_questionCardPanel.isCardSideVisible(1);
        }
