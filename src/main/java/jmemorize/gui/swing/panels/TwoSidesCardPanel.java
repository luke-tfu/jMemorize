...
public class TwoSidesCardPanel extends CardPanel implements CardFontObserver {
    ...
    /**
     * Enable/Disable flipped card sides mode. Enabling this mode will use the front side font on the flip side panel
     * and the flip side font on the front side panel.
     * 
     * (This is needed because CardPanel is also used to present the answers in learn sessions.)
     * 
     * @param enable
     *            <code>true</code> to enable flipped sides mode. <code>false</code> to disable.
     */
    ...
    public boolean isValidCard() {
        boolean validFront = m_frontSide.getText().getUnformatted().length() > 0 || m_frontSide.getImages().size() > 0;

        boolean validBack = m_backSide.getText().getUnformatted().length() > 0 || m_backSide.getImages().size() > 0;
