 ...
 * 
 *         This class extends JProgressBar in order to show two values for progress, which are presumably related so
 *         that they can be shown on a stacked bar.
 * 
 *         The underlying data object, the BoundedRangeModel, already has a data member, extent, representing the length
 *         of an inner range, so all we have to do is expose methods for adjusting the value of extent and implement a
 *         paint method which paints the inner range in a separate color.
 * 
 *         The drawback to this painting implementation is that if isStringPainted, then the rectangle representing the
 *         extent paints over the text. - Workaround - if you want the the text to be visible through the extent
 *         rectangle, set the extentForeground to a partially transparent color. See the included main for an example.
 * 
 *         TODO - fixing this requires fully implementing paint (background, both rectangles, and the text in three
 *         colors).
 * 
