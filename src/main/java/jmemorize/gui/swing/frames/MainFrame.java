...
/**
 * The main window of jMemorize. It has a stats panel in the upper part and a card table/learn panel in the bottom.
 * Optionaly there is also a category tree at the left side.
 * 
 * @author djemili
 */
public class MainFrame extends JFrame
        implements CategoryObserver, SelectionProvider, SelectionObserver, LearnSessionObserver, ProgramEndObserver {
    ...
    /*
     * Simply listen for card selection events in learn and decktable panel and forward them.
     */
     ...
     * 
     * @param level
     *            the level of the deck that is to be shown. The deck with unlearned cards has level 0.
     */
    ...
    /**
     * @return the level of the currently shown deck. The deck with unlearned cards has level 0.
     */
    public int getDeck() {
        return m_deck;
    }
    ...
    /**
     * @param show
     *            <code>true</code> if the category tree is supposed to be shown. <code>false</code> otherwise.
     */
    ...
    public void startLearning(Category category, List<Card> selectedCards, boolean learnUnlearned,
            boolean learnExpired) {
    ...
    /**
     * Loads the lesson and sets it as currently loaded lesson. If there is already a opened lesson, the user might be
     * prompted to save that lesson before opening the new lesson.
     * 
     * @param file
     *            The path to the lesson. If <code>null</code> a file chooser is shown that allows the user to select
     *            the file.
     */
    ...
    public void saveLesson(Lesson lesson, File file) {
        try {
            if (file == null) {
                file = AbstractExportAction.showSaveDialog(this, MainFrame.FILE_FILTER);
    ...
    /**
     * If lesson was modified this shows a dialog that asks if the user wants to save the lesson before closing it.
     * 
     * @return <code>true</code> if user chose not to cancel the lesson close operation. If this method return
     *         <code>false</code> the closing of jMemorize was canceled.
     */
    ...
    public void onProgramEnd() {
        ...
        int vSize = m_verticalSplitPane.getDividerLocation() > 0 ? m_verticalSplitPane.getDividerLocation()
    ...
    private JPanel buildCategoryBar() {
        ...
        JLabel categoryLabel = new JLabel(Localization.get(LC.CATEGORY), SwingConstants.CENTER);
