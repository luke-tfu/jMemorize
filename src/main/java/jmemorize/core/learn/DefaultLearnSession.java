...
/**
 * A learn session is instantiated with a LearnSettings object which defines the rules which the session should handle
 * cards.
 * 
 * The workflow for this class is as following:
 * 
 * <ol>
 * <li>Learn Session fetches a card according to its LearnSettings. To get notified of this card, use the
 * {@link LearnCardObserver}.</li>
 * <li>The learn session waits for a call to either {{@link #cardChecked(boolean, boolean)} or {@link #cardSkipped()}.
 * This makes the learn session perform some action on the card.</li>
 * <li>This action results in some category event, which the learn session gets notified of, because it attached itself
 * also as a category observer to the category that is to be learned. This category event results in either fetching the
 * next card (see above) or all call to the {@link LearnSessionProvider} to inform it that the session has ended. The
 * {@link LearnSessionProvider} then notifies all of its {@link LearnSessionObserver}.</li>
 * </ol>
 * 
 * Note that when a card is neither learned or skipped, but i.e. deleted or resetted, step 2 is skipped and step 3 comes
 * into play directly.
 * 
 * The order of a card in the learn session depends on the shuffle and category order settings: [Shuffle: Off, Category
 * Order: Off] Deck, Last test date [Shuffle: Off, Category Order: On ] Category, Deck, Last test date [Shuffle: On,
 * Category Order: Off] Deck, Random number [Shuffle: On, Category Order: On ] Category, Deck, Random number
 * 
 * @author djemili
 */
public class DefaultLearnSession implements CategoryObserver, LearnSession {
    /**
     * A Comparator that is used for sorting cards in learn sessions. This is used to sort the cards into equivalence
     * classes, from which the next card will be drawn randomly.
     */
    ...
    /**
     * This class is a wrapper for a card. It allows to associate additional data to a card, that is only relevant
     * during a single specifc learn session.
     */
    private class CardInfo {
        private Card m_card;

        /**
         * For learning this variable should be used instead of the real level of the card. This allows for some special
    ...
    /**
     * Creates a new learn session. Use {@link #startLearning()} to start the learning.
     */
    public DefaultLearnSession(Category category, LearnSettings settings, List<Card> selectedCards,
            boolean learnUnlearned, boolean learnExpired, LearnSessionProvider provider) {
        ...
        Map<Category, Integer> order = m_settings.isGroupByCategory() ? createCategoryGroupOrder() : null;
    ...
    public void startLearning() {
        ...
        if (m_settings.isCardLimitEnabled() && m_cardsActive.size() > m_settings.getCardLimit()) {
    ...
    public void cardChecked(boolean passed, boolean shownFlipped) {
        ...
        m_logger.fine(String.format("cardChecked: %b %s", passed, currentCard.getFrontSide().getText()));
        ...
        if (passed) {
            ...
            /*
             * NOTE - If the card is still active, the card may be in the wrong equivalence class (i.e. the set is in an
             * inconsistent state). We can't fix it until after the reset, *but* the resetCardLevel() method fires the
             * event which results in the observers reacting (checking for end of session, getting the next card, etc).
             * The card's equivalence class will be wrong, but this should not be a problem for gotoNextCard. We reset
             * the equivalence class as soon as possible.
             */
    ...
    public void cardSkipped() {
        ...
        if (m_cardsReserve != null && m_cardsReserve.size() > 0) {
            ...
            if (replacementCard.getLearnedAmount(true) > 0 || replacementCard.getLearnedAmount(false) > 0) {
    ...
    /**
     * Note that this method is specialy for DefaultLearnSession and not part of the LearnSession interface.
     * 
     * @return the shuffled 'fake' card level that is currently used for the card.
     */
     ...
     * 
     * @return <code>true</code> if the card should be flipped. <code>false</code> otherwise.
     */
    private boolean checkIfFlipped() {
        if (m_settings.getSidesMode() == LearnSettings.SIDES_RANDOM) {
            ...
            int timesToLearnFront = m_settings.getAmountToTest(true) - currentCard.getLearnedAmount(true);

            int timesToLearnBack = m_settings.getAmountToTest(false) - currentCard.getLearnedAmount(false);
    ...
    /**
     * Fetch the cards that should be learned in this session according to given params.
     */
    private EquivalenceClassSet<CardInfo> fetchCards(List<Card> selectedCards, boolean learnUnlearnedCards,
            boolean learnExpiredCards, Map<Category, Integer> categoryGroupOrder) {
        ...
        EquivalenceClassSet<CardInfo> cardSet = new EquivalenceClassSet<CardInfo>(
    ...
    /**
     * @return Cards that are being learned can be grouped by categories. In this case the map holds for every category
     *         the position when it should appear.
     */
