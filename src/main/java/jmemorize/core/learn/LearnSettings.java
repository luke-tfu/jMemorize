...
/**
 * This class holds learn session settings which can be saved and loaded. This class is used when creating a learn
 * session and defines the strategy that should be used while in that learn session. You can enable limits, schedules
...
public class LearnSettings {
    public enum SchedulePreset {
        CONST, LINEAR, QUAD, EXPONENTIAL, CRAM, CUSTOM
    }

    // schedules
    public static final int SCHEDULE_LEVELS = 10;
    public static final String[] SCHEDULE_PRESETS = new String[] { Localization.get(LC.SCHEDULE_CONST),
            Localization.get(LC.SCHEDULE_LINEAR), Localization.get(LC.SCHEDULE_QUAD),
            Localization.get(LC.SCHEDULE_EXPONENTIAL), Localization.get(LC.SCHEDULE_CRAM),
     ...
     * 
     * @param enabled
     *            <code>true</code> if the card limit should be enabled. <code>false</code> otherwise.
     */
     ...
     * 
     * @param enabled
     *            <code>true</code> if the time limit should be enabled. <code>false</code> otherwise.
     */
    ...
    /**
     * @param retest
     *            <code>true</code> if cards that have been failed while learning should be put back into the list of
     *            cards to learn. <code>false</code> if all cards should never be tested more then once in a session.
     */
     ...
     * 
     * @return <code>true</code> if failed cards can appear more then once in a session.
     */
     ...
     * <li>SIDES_RANDOM: show cards with randomly selected NORMAL or FLIPPED sides mode.</li>
     * <li>SIDES_BOTH: show both sides of cards as specified in {@link #setAmountToTest(boolean, int)}.</li>
     * </ul>
     * 
     * @param mode
     *            either SIDES_NORMAL, SIDES_FLIPPED, SIDES_RANDOM or SIDES_BOTH;
     */
    ...
    /**
     * @return The current sides mode as given by enum SIDES_NORMAL, SIDES_FLIPPED and SIDES_RANDOM.
     */
    ...
    /**
     * @param frontside
     *            True if you want to get the amount to test the front
     * 
     * @return The amount you need to test a given side before it is declared learnt
     */
    ...
    /**
     * @param frontside
     *            True if you want to set the amount to test the front
     * @param value
     *            The number of correct consecutive tests before it is declared learnt
     */
    ...
    /**
     * The schedule tells how much time should pass before a cards that has moved into a higher deck level needs be
     * rechecked.
     * 
     * @param schedule
     *            A int array that holds the time span values that need to pass and where the index is the deck level
     *            before the card moved - e.g. <code>schedule[0] = 60</code> says that a card that has moved from deck 0
     *            to deck 1 should be rechecked in one hour. The time spans are given in minutes.
     */
     ...
     * 
     * @param idx
     *            the index of the schedule preset. See {@link #SCHEDULE_PRESETS}
     */
    ...
    /**
     * @param hour
     *            needs to be given in 24-hour format.
     */
    ...
    /**
     * @return the correct expiration date according to the current schedule settings.
     * 
     * @param learnDate
     *            The moment that the card is learned.
     * @param currentLavel
     *            The deck level of the card before raising it to the next level.
     */
    public Date getExpirationDate(Date learnDate, int currentLevel) {
        ...
        if (m_fixedExpirationTimeEnabled) {
            ...
            if (hour > m_fixedExpirationHour || (hour == m_fixedExpirationHour && minute >= m_fixedExpirationMinute)) {
     ...
     * 
     * @param enable
     *            <code>true</code> if grouping should be enabled. <code>false</code> otherwise.
     */
    ...
    /**
     * This method sets the order by which categories will be shown when grouping by categories is enabled.
     * 
     * @param order
     *            Is either CATEGORY_ORDER_CARDS or CATEGORY_ORDER_FIXED.
     */
    ...
    /**
     * 0.0f means that all cards appear in the order of their level. 1.0f means that all cards appear in totally random
     * order.
     * 
     * Something in between denotes the share of cards that will be learned at a random level value.
     */
    ...
    /**
     * The share of cards that will appear at a random level in the learn session.
     */
     ...
     * 
     * @param idx
     *            the index of the preset schedule. See {@link #SCHEDULE_PRESETS}
     * 
