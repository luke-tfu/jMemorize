...
public class LearnHistory {
    public class SessionSummary implements Cloneable {
        ...
        private SessionSummary(Date start, Date end, float passed, float failed, float skipped, float relearned) {
            this(start, end, (int) ((end.getTime() - start.getTime()) / (1000 * 60)), passed, failed, skipped,
                    relearned);
        }
        ...
        private SessionSummary(Date start, Date end, int duration, float passed, float failed, float skipped,
                float relearned) {
            m_start = start;
            m_end = end;
            m_duration = duration;
        ...
        public boolean equals(Object obj) {
            ...
            return m_passed == other.m_passed && m_failed == other.m_failed && m_skipped == other.m_skipped
    ...
    public void addSummary(Date start, Date end, int passed, int failed, int skipped, int relearned) {
        SessionSummary sessionSummary = new SessionSummary(start, end, passed, failed, skipped, relearned);
    ...
    public List<SessionSummary> getSummaries(CalendarComparator comp) {
        ...
        for (SessionSummary summary : m_summaries) {
            if (lastSummary == null || comp.compare(summary, lastSummary) != 0) {
                ...
                aggregatedSummary = new SessionSummary(aggregatedSummary.m_start, summary.m_end,
                        aggregatedSummary.m_duration + summary.m_duration,
                        aggregatedSummary.m_passed + summary.m_passed, aggregatedSummary.m_failed + summary.m_failed,
    ...
    public List<SessionSummary> getSummaries(CalendarComparator comp, int limit, boolean showEmpty) {
    ...
    public SessionSummary getAverage() {
        ...
        if (count > 0) {
            return new SessionSummary(summary.getStart(), summary.getEnd(), (int) (summary.getDuration() / count),
                    summary.getPassed() / count, summary.getFailed() / count, summary.getSkipped() / count,
                    summary.getRelearned() / count);
        } else {
            return new SessionSummary(new Date(), new Date(), 0, 0, 0, 0);
        }
    }
    ...
    public SessionSummary getSummary(Date date, CalendarComparator comp) {
        ...
        return !found ? null : new SessionSummary(date, date, duration, passed, failed, skipped, relearned);
    }
    ...
    public SessionSummary getSessionsSummary() {
        ...
        return new SessionSummary(first.getStart(), last.getEnd(), duration, passed, failed, skipped, relearned);
    }
