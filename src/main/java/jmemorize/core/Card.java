...
public class Card implements Events, Cloneable {
    ...
    public void setSides(FormattedText front, FormattedText back) throws IllegalArgumentException {
        if (front.equals(m_frontSide.getText()) && back.equals(m_backSide.getText())) {
            return;
        }
    ...
    /**
     * Get the number of times a specific card side was already learned in its deck.
     * 
     * @param frontside
     *            <code>true</code> if it should deliver the fronside value, <code>false</code> if it should deliver the
     *            backside value.
     * 
     * @return the amount of times that the specified side was learned in this deck.
     */
    ...
    /**
     * Set the number of times a specific card side was already learned in its deck.
     * 
     * @param frontside
     *            <code>true</code> if it should deliver the fronside value, <code>false</code> if it should deliver the
     *            backside value.
     * 
     * @param amount
     *            the amount of times that the specified side was learned in this deck.
     */
    ...
    /**
     * Increment the number of times a specific card side was already learned in its deck by one.
     * 
     * @param frontside
     *            <code>true</code> if it should deliver the fronside value, <code>false</code> if it should deliver the
     *            backside value.
    ...
    /**
     * Resets the amount of times that the card sides were learned in this deck to 0.
     */
    ...
    /**
     * @return the date that this card appeared the last time in a test and was either passed or failed (skip doesn't
     *         count).
     */
    ...
    /**
     * @return DateTouched is the date that this card was learned, skipped, reset or created the last time. This value
     *         is used to sort cards by a global value that is unique for all categories and decks.
     */
    ...
    /**
     * @return The percentage of times that this card has passed learn tests in comparison to failed tests.
     */
    ...
    /**
     * A card is expired when it was learned/repeated succesfully, but its learn time has expired (is in the past from
     * current perspective).
     * 
    ...
    public boolean isExpired() {
        ...
        return m_dateExpired != null && (m_dateExpired.before(now) || m_dateExpired.equals(now));
    }

    /**
     * A card is learned when it was learned/repeated succesfully and its learn time hasnt expired.
     * 
    ...
    /**
     * A card is unlearned when it wasnt succesfully repeated or never been l earned at all.
     * 
     ...
     * 
     * This includes the following data: Fronside, Flipside, Creation date. Setting the right category needs to be
     * handled from the out side.
     */
    public Card cloneWithoutProgress() {
        try {
            return new Card(m_dateCreated, (CardSide) m_frontSide.clone(), (CardSide) m_backSide.clone());
        } catch (CloneNotSupportedException e) {
            assert false;
    ...
    /**
     * @return clone of given date or <code>null</code> if given date was <code>null</code>.
     */
