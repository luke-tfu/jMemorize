...
/**
 * A card category can hold a number of 0 to n decks. The cards in deck 0 are called <b>unlearned</b>. Cards in higher
 * decks are called <b>learned</b> or <b>expired</b>, depending on the fact of wether their expiration timer has already
 * passed by or not.
 * 
 * A category can have a number of child categories. Fetching cards from a parent category will also fetch all cards of
 * its child categories.
 * 
 * For example category B is child of category A. A holds a card CA and B holds a card CB. Getting all cards of category
 * A will then return CA <i>and</i> CB.
 * 
 * Observers can be hooked to categories and will be notified when a card or category event in this category or one of
 * its child categories happens.
 * 
 * @author djemili
 */
public class Category implements Events {
    ...
    /**
     * Adds a card to the level 0 deck of this category. The card will be considered as unlearned after being added to
     * deck level 0.
     * 
     * Fires a ADDED_EVENT.
     */
    ...
    /**
     * Moves the card to a new category, preserving all its fields and its level. This is different from removing and
     * then adding a card because it triggers a single MOVE event instead of a REMOVE and ADD event.
     */
    ...
    /**
     * Removes the card from its current deck and adds it to the next deck. The given date is used as new expiration
     * date.
     * 
     * Fires a DECK_EVENT.
     */
    ...
    /**
     * Removes the card from its current deck and appends it to deck 0 (even if its already at level 0). TotalTests is
     * increased by one.
     * 
     * Fires a DECK_EVENT.
     */
    ...
    /**
     * Removes the card from its current deck and reappends it to the same deck again. This doesnt change any values
     * besides DateTouched.
     * 
     * Fires a DECK_EVENT.
     */
     ...
     *            the deck level.
     * 
     * @return all cards in the given deck level in this category and its child categories. Returns all cards of all
    ...
    /**
     * @return all expired cards of all decks in this category and its child categories.
     */
    ...
    /**
     * @return all expired cards of given deck in this category and its child categories.
     */
    ...
    /**
     * @return all learned cards of all decks in this category and its child categories.
     */
    ...
    /**
     * @return All cards that are local to this category. That is all cards that directly belong to this category and
     *         not to any of this child categories.
     */
    ...
    /**
     * @return All cards in the level that are local to this category. That is all cards that directly belong to this
     *         category and not to any of this child categories.
     */
    ...
    /**
     * @return The number of decks of this category and its child categories. That means that no child categoriy can
     *         have more number of decks then its parent category.
     */
    ...
    /**
     * @return the child category with given name. <code>null</code> if there is child category with given name.
     */
    ...
    /**
     * @return The parent of this category or <code>null</code> if it has no parent.
     */
    ...
    /**
     * @return a textual representation of the category path of this category. Starting from the root, every category in
    ...
    /**
     * @return a clone of this category. The clone contains the same child categories and the same cards as this
     *         category, but without any user dependent-stats(e.g. all cards are at level 0 and have no date_tested).
     */
