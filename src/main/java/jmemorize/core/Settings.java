...
/**
 * Stores all jMemorize settings by using a preferences node. Use storeXXX to save settings and loadXXX to load them
 * again.
 * 
 * @author djemili
 */
public class Settings {
    /**
     * Listens for changes to the currently set card font which can be set by using the preferences window.
     */
    ...
    /**
     * @return The locale stored in preferences or the default locale if no locale was stored.
     */
    ...
    public static void setCardFont(CardFontObserver observer, boolean flipped, FontType type1, FontType type2) {
    ...
    public static void setCardFont(CardFontObserver observer, FontType type1, FontType type2) {
    ...
    /**
     * @return A newly created strategy that got instantiated with all the stored strategy data or with default values.
     */
    public static LearnSettings loadStrategy(MainFrame frame) {
        ...
        settings.setFixedExpirationTimeEnabled(PREFS.getBoolean(SCHEDULE_FIXED_EXPIRATION_ENABLED, false));
        ...
        settings.setCategoryOrder(PREFS.getInt(CATEGORY_ORDER, LearnSettings.CATEGORY_ORDER_FIXED));
    ...
    /**
     * @return <code>true</code> if lesson files should be compressed with GZIP when saving.
     */
    ...
    public static void storeFrameState(JFrame frame, String frameId) {
        Settings.storeIsFrameMaximized(frameId, frame.getExtendedState() == Frame.MAXIMIZED_BOTH);
     ...
     * 
     * @return <code>true</code> if the main frame should be displayed as maximized at program start.
     */
     ...
     *            an identifier for the frame.
     * 
     * @return the position of the main frame. <code>null</code> if no position is stored.
     */
     ...
     *            an identifier for the frame.
     * 
     * @return the size of the main frame. <code>null</code> if no size is stored.
     */
    ...
    private static void storeFrameSize(String frameId, Dimension size) {
        PREFS.put(frameId + '.' + FRAME_SIZE, (int) size.getWidth() + "," + (int) size.getHeight());
    }
