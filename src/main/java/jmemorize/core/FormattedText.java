...
/**
 * This class handles encoding/decoding and displaying formatted/ unformatted text. The text is immutable.
 * 
 * Styled document Unformatted String <--> FormattedText class Encoding
 * 
 * @author djemili
 */
public class FormattedText implements Cloneable {
    ...
    private static final Pattern TEXT_PATTERN = Pattern.compile("(.*?)<(/?(b|i|u|sub|sup)?)>", Pattern.DOTALL);
    ...
    public static FormattedText formatted(StyledDocument document) {
        ...
        String fText = removeRedundantTags(getFormattedText(root, 0, document.getLength()));

        return FormattedText.formatted(fText);
    }

    public static FormattedText formatted(StyledDocument document, int start, int end) {
        Element root = document.getDefaultRootElement();
        String fText = removeRedundantTags(getFormattedText(root, start, end));
    ...
    private static String removeRedundantTags(String formattedText) {
        /*
         * StyledDocument merges styles in certain situations. To avoid that this results in getting a different
         * encoding after decoding to a StyledDocument and back to an encoding again, we remove redundant tags by
         * ourself.
         */
    ...
    private static String getFormattedText(Element e, int startSelection, int endSelection) {
        StringBuffer sb = new StringBuffer();
        ...
        // }
            ...
            for (int i = 0; i < e.getElementCount(); i++) {
                sb.append(getFormattedText(e.getElement(i), startSelection, endSelection));
            }
        }
    ...
    private void decode(StyledDocument doc, String text, int offset) throws BadLocationException, ParseException {
        StringBuffer sb = new StringBuffer(text);

        // Map<Integer, ImageIcon> images = decodeImages(doc, sb);

        /*
         * problem we need to decode the images first and remove the strings from the overall string, because pattern
         * searching takes too long. this is problematic though, because decodeImages expects to be called afterwards.
         */
