...
public class CsvBuilder {
    ...
    /**
     * Exports the given lesson to a CSV-file with given delimiter and given character set.
     */
    public static void exportLesson(OutputStream out, Lesson lesson, char delimiter, Charset charset)
            throws IOException {
        try {
    ...
    /**
     * Parses the given file that holds text values that are delimited by given delimiter. The values are used to
     * contruct a lesson.
     * 
     * @param delimiter
     *            the delimiter that is used to separate values in the file.
     * @param charset
     *            the character set that the file contents are using.
     * @param file
     *            the file that should be read.
     * @param charset
     *            the character set that is used in the file. Use <code>null</code> to use the default charset.
     * @param the
     *            lesson to which the contents of the file will be added.
     * 
     * @throws IOException
     *             if the file couldn't be read or the values are malformatted.
     */
    public static void importLesson(InputStream in, char delimiter, Charset charset, Lesson lesson)
        ...
        try {
            ...
            while (reader.readRecord()) {
                ...
                if (frontSide.getUnformatted().length() == 0 || flipSide.getUnformatted().length() == 0)
                    throw new IOException("You have to specify at least a front " + "side and flip side for every card "
                            + getLineString(reader) + ".");

                Card card = new Card(frontSide, flipSide);

                Category category;
                String categoryName = reader.get(CATEGORY_COL);
                if (categoryName.length() == 0 || categoryName.equalsIgnoreCase(rootCategory.getName())) {
    ...
    private static void writeHeader(CsvWriter writer) throws IOException, com.csvreader.CsvWriter.FinalizedException {
    ...
    private static void validateHeader(String[] headers) throws BadHeaderException {
        ...
        for (int i = 0; i < headers.length; i++) {
            ...
            if (!headers[i].equalsIgnoreCase(CATEGORY_COL) && !headers[i].equalsIgnoreCase(LEVEL_COL))
                throw new BadHeaderException("Unknown header column: " + headers[i]);
        }

        if (!hasFront || !hasFlip)
            throw new BadHeaderException(
                    "The first line needs to specify " + "the header, which must have at least contain the columns '"
